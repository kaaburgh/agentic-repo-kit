...
- **Validation evidence:** A synthetic reconstruction of the reviewed shape (34 items, one never-validated gate, a 22-item chain, two unbounded ready items) reports `ready = 2/34 outstanding (5.9%)` and `chokepoint: ENV1 gates 31/34 outstanding (91.2%)` — the two lines that were the entire conclusion of a multi-hour manual review — while `check` still returns ok. Full unit tests (124) and `python -m agentic_repo_kit check .` pass locally on the working head. The `Agentic repository contract` workflow run `32890178223` passed on exact head `b91475ac49683ea960db5dc46702b4f726245cfe`, building the fallback artifact, verifying its digest against the lock and running `check` from the pinned artifact. The unit-test workflow does not trigger on branch pushes (`ci.yml` runs on `pull_request` and pushes to `main`), so unit tests in CI on the exact head remain outstanding and this item stays validation-incomplete until then.
- **Validation / acceptance:**
...
- **Validation evidence:** Focused tests cover both field shapes, the parameterised member, an unresolvable blocker ID, project extension, the default-warn and enforced-fail migration paths, and the component-count mismatch. Both established dogfood presentation shapes still validate. Full unit tests (124) and `python -m agentic_repo_kit check .` pass locally on the working head. The `Agentic repository contract` workflow run `32890178223` passed on exact head `b91475ac49683ea960db5dc46702b4f726245cfe`, building the fallback artifact, verifying its digest against the lock and running `check` from the pinned artifact. The unit-test workflow does not trigger on branch pushes (`ci.yml` runs on `pull_request` and pushes to `main`), so unit tests in CI on the exact head remain outstanding and this item stays validation-incomplete until then.
- **Validation / acceptance:**
